...
| `error` | 项目错误信息 |
| `visibility` | 项目总览显示状态：`active` / `hidden` |
| `hidden_at` | 项目被隐藏的时间 |
| `hidden_reason` | 隐藏原因，当前预留 |
| `created_at` | 创建时间 |
| `updated_at` | 更新时间 |
...
### 9.9 POST /api/projects/:handle/visibility

按 X 账号更新项目总览显示状态。隐藏操作不删除数据库记录，也不影响历史报告。
...
行为：
- 同一 X 账号的历史项目记录会一起更新，避免隐藏最新记录后旧记录重新出现在总览。
- 项目汇总默认只显示 `active` 项目。
- 顶部“显示”筛选支持 `活跃 / 已隐藏 / 全部`。
- 已隐藏账号重新输入分析时，不会被默认静默跳过；新批次会把该账号恢复为 `active`。
...
| 项目隐藏 / 恢复 | 已实现 | 可在项目总览隐藏已分析完成但暂不关注的账号；隐藏不删除历史报告，重新分析会自动恢复为活跃 |
