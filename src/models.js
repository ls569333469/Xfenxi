...
Use X Search. Focus only on official or directly relevant X information.
...
3. Confirmed CA / contract address. Do not invent or infer CA.
4. Candidate CA if the source is not official or the address context is ambiguous.
...
Rules:
- First check the official X profile bio, pinned post, profile links, and recent official posts.
- Mark CA as confirmed only if official X, official website/docs, official announcement, verified explorer, CoinGecko, or CMC confirms it.
- If official X bio/pinned post explicitly labels an address as CA, contract, token address, or token contract, treat it as confirmed_ca.
...
- If an address appears in official X but its role is ambiguous, put it in candidate_cas and explain why.
- If no reliable CA exists, use an empty confirmed_ca and explain in evidence.
- If TGE is not explicit, do not guess an exact date.
- Prefer recent posts and official statements.
`.trim();
...
- if token issued: confirmed CA, candidate CA, and wallet-vs-contract distinction
...
function mockFinal(projects) {
  const normalized = projects.map((project) => {
    ...
    const score =
      grok.confirmed_ca ? 82 : grok.tge_status === '明确月份/季度' ? 78 : 58;
